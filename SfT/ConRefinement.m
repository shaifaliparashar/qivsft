function phi=ConRefinement(p,q,phi,options)
if(nargin<3)
...
X=phi.L;
[cost,costgrad,Hess]=Concost(X,p,q,phi.C,phi.er,options.isoer,phi.EpsilonLambda,phi.p,options.delta);
...
while(found==0 & iter<options.maxiter)
    ...
    Hessant=Hess;
    Xant=X;
    while(rcond(Hess+nu.*eye(size(Hess)))<1e-8)
    nu=nu*2;
    end
    XDelta=reshape(-inv(Hess+nu.*eye(size(Hess)))*costgrad,size(X));
[cost,costgrad,Hess]=Concost(X+XDelta,p,q,phi.C,phi.er,options.isoer,phi.EpsilonLambda,phi.p,options.delta);
    if(cost<costant)
...
found=(found|(nu>1e10));
if(options.verbose)
 disp(sprintf('[RECON-PHI]iter=%d;cost=%f;nu=%f;diff=%f',iter,cost,nu,norm(costgrad)))
end
    iter=iter+1;
end
phi.L=X;
